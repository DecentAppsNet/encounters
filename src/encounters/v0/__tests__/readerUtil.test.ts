...
describe('readerUtil', () => {
  ...
  it('reads an encounter text', () => {
    ...
    expect((a as InstructionMessageAction).messages.nextMessage()).toBe('You are a troll guarding a bridge.');
    ...
    a = c.actions[0];
    expect(a.actionType).toBe(ActionType.NARRATION_MESSAGE);
    expect((a as DisplayMessageAction).messages.nextMessage()).toBe('The troll reluctantly allows you to pass. Victory!');
  });
});